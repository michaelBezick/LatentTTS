...
export MODEL_DTYPE
export SEED
export ACCEPTANCE_MARGIN
export SPLICE_SCORE_CHUNK_SIZE
export SPLICE_DECODE_BATCH_SIZE

mkdir -p "${LOG_DIR}"

sbatch_args=(
    "--job-name=latenttts-latent-splice-eval"
    "--nodes=1" "--ntasks=1"
    "--chdir=${REPO_ROOT}"
...
echo "Submitting latent splice eval job..."
sbatch "${sbatch_args[@]}" "${SCRIPT_DIR}/run_latent_splice_eval.sbatch"
